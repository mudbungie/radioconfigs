...
universaldirectives = []
with open('universalconfig.conf', 'r') as universalconfig:
...
with open('specifics.conf', 'w') as output:
    for tower in os.listdir('configs'):
        towername = tower.replace('.cfg', '')
        output.write('BEGIN ' + towername + '\n')
        with open('configs/' + tower, 'r') as towerconfig:
...
for tower in os.listdir('configs'):
    towername = tower.replace('.cfg', '')
    with open('newconfigs/' + tower, 'w') as output:
        with open('specifics.conf') as specifics:
            start = False
            for line in specifics.readlines():
                ...
                if line == 'BEGIN ' + towername + '\n':
                    start = True
        with open('universalconfig.conf') as universal:
